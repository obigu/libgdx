...
class OALIOSAudioDevice implements AudioDevice {
    ...
    OALIOSAudioDevice(int samplingRate, boolean isMono, int minSize, int bufferCount) {
        ...
        for (int i = 0; i < bufferCount; i++) {
            ALBuffer buffer = new ALBuffer().initWithNameDataSizeFormatFrequency("test", Struct.allocate(VoidPtr.class, 1), 2, format, samplingRate);
    ...
    public void writeSamples(short[] samples, int offset, int numSamples) {
        if (numSamples < 0)
            throw new IllegalArgumentException("numSamples cannot be < 0.");
        ShortPtr shortPtr;
