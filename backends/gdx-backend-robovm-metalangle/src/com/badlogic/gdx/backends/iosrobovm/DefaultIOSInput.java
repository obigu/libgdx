...
public class DefaultIOSInput extends AbstractInput implements IOSInput {
    ...
    private final UITextViewDelegate textViewDelegate = new UITextViewDelegateAdapter() {
        ...
        public void didChange(UITextView textView) {
            if (textView.getText().isEmpty()) {
                ...
                textView.setSelectedTextRange(textView.getTextRange(textView.getBeginningOfDocument(), textView.getBeginningOfDocument()));
            }
        }
        ...
        public boolean shouldChangeCharacters(UITextView textView, NSRange range, String text) {
            ...
            if (nativeInputConfiguration.getMaxLength() != -1 && textView.getText().length() + (text.length() - range.getLength()) > nativeInputConfiguration.getMaxLength()) {
                return false;
            }
            if (nativeInputConfiguration.getValidator() == null)
                return true;
    ...
    private final UITextFieldDelegate textDelegate = new UITextFieldDelegateAdapter() {
        ...
        public boolean shouldChangeCharacters(UITextField textField, NSRange range, String text) {
            if (nativeInputConfiguration.getMaxLength() != -1 && textField.getText().length() + (text.length() - range.getLength()) > nativeInputConfiguration.getMaxLength()) {
                return false;
            }
            if (nativeInputConfiguration.getValidator() == null)
                return true;
    ...
    public void setOnscreenKeyboardVisible(boolean visible, OnscreenKeyboardType type) {
        ...
        if (textfield == null)
            createDefaultTextField(false, false);
    ...
    public void openTextInputField(final NativeInputConfiguration configuration) {
        if (textfield != null)
            throw new GdxRuntimeException("Can't open TextInputField, if KeyBoard is already open");
        configuration.validate();
        this.nativeInputConfiguration = configuration;
        createDefaultTextField(configuration.isMultiLine(), configuration.isMultiLine() || (configuration.getType() != OnscreenKeyboardType.Default && configuration.getType() != OnscreenKeyboardType.Password));
        ...
        if (textfield instanceof UITextView) {
            ...
            if (configuration.getAutoComplete() != null) {
                suggestionTable = new UITableView(new CGRect(app.graphics.screenBounds.width, app.graphics.screenBounds.height, app.graphics.screenBounds.width, 50));
                ...
                suggestionTable.setDataSource(new UITableViewDataSourceAdapter() {
                    ...
                    public UITableViewCell getCellForRow(UITableView tableView, NSIndexPath indexPath) {
                        ...
                        if (cell == null)
                            cell = new UITableViewCell(UITableViewCellStyle.Default, "suggestion");
                        if (Foundation.getMajorSystemVersion() >= 14) {
                            UIListContentConfiguration contentConfiguration = cell.defaultContentConfiguration();
                            NSAttributedString coloredText = new NSAttributedString(available.get(indexPath.getRow()), new NSDictionary<>(NSAttributedStringAttribute.ForegroundColor.value(), UIColor.white()));
                ...
                asTextField.addTarget(new NSObject() {
                    ...
                    public void changedText(UITextField textField) {
                        ...
                        suggestionTable.setFrame(new CGRect(new CGPoint(textFrame.getX(), textFrame.getY() - height), new CGSize(textFrame.getWidth(), height)));
            ...
            NSAttributedString placeholderString = new NSAttributedString(configuration.getPlaceholder(), new NSDictionary<>(NSAttributedStringAttribute.ForegroundColor.value(), UIColor.lightGray()));
        ...
        UITextPosition start = uiTextInput.getPosition(uiTextInput.getBeginningOfDocument(), configuration.getTextInputWrapper().getSelectionStart());
        UITextPosition end = uiTextInput.getPosition(uiTextInput.getBeginningOfDocument(), configuration.getTextInputWrapper().getSelectionEnd());
    ...
    public void closeTextInputField(boolean isConfirmative, @Null NativeInputCloseCallback callback) {
        ...
        long selectionStart = uiTextInput.getOffset(uiTextInput.getBeginningOfDocument(), uiTextInput.getSelectedTextRange().getStart());
        long selectionEnd = uiTextInput.getOffset(uiTextInput.getBeginningOfDocument(), uiTextInput.getSelectedTextRange().getEnd());
        ...
        Gdx.app.postRunnable(() -> {
            ...
            if (callback != null)
                keepOpen |= callback.onClose(isConfirmative);
        });
    ...
    private void createDefaultTextField(boolean isMultiLine, boolean needsDoneToolbar) {
        ...
        if (needsDoneToolbar) {
            uiToolbar = new UIToolbar(new CGRect(new CGPoint(0, 0), new CGSize(UIScreen.getMainScreen().getBounds().getSize().getWidth(), 35)));
    ...
    public boolean isPeripheralAvailable(Peripheral peripheral) {
        ...
        if (peripheral == Peripheral.Vibrator)
            return haptics.isVibratorSupported();
        if (peripheral == Peripheral.HapticFeedback)
            return haptics.isHapticsSupported();
        ...
        synchronized (touchEvents) {
            ...
            for (TouchEvent event : touchEvents) {
                ...
                switch(event.phase) {
                    ...
                    case Cancelled:
                        if (inputProcessor != null)
                            inputProcessor.touchCancelled(event.x, event.y, event.pointer, Buttons.LEFT);
                        break;
                    case Moved:
                    case Stationary:
                        if (inputProcessor != null)
